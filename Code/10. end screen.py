...
while RESTART:
    ...
    while GAMESTART:
        ...
        pygame.draw.rect(windowSurface, WHITE, (WINDOWWIDTH/2-100, WINDOWHEIGHT/2-30, 200, 60))
        mousex = 0
        mousey = 0
    ...
    while WINNER:
        ...
        if HP1 < 0:
            HP1 += 10
            '''text = basicFont.render('Player2 WIN!', True, WHITE, GRAY)
        ...
        if HP2 < 0:
            HP2 += 10
            '''text = basicFont.render('Player1 WIN!', True, WHITE, GRAY)
            textRect = text.get_rect()
            textRect.centerx = windowSurface.get_rect().centerx
            textRect.centery = windowSurface.get_rect().centery'''
            WINNER = 0
    ...
    while GAMEEND:
        ...
        pygame.draw.rect(windowSurface, BLACK, (WINDOWWIDTH/2-100, WINDOWHEIGHT/2-30, 200, 60))
        mousex = 0
        mousey = 0
        for event in pygame.event.get():
            if event.type == QUIT:
