...
while RESTART:
    ...
    while WINNER:
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYUP:
                ...
                if event.key == ord('l'):
                    pplayer2.top = random.randint(0, WINDOWHEIGHT - pplayer2.height)
                    pplayer2.left = random.randint(0, WINDOWWIDTH - pplayer2.width)
        ...
        if fireGun1 and BULLETINTERVAL1 == BULLETGAP:
            if direction1 == 'left':
                left1.append(pygame.Rect(pplayer1.left+(SIZE1/2)-BULLETSIZE, pplayer1.centery-(BULLETSIZE/2), BULLETSIZE, BULLETSIZE))
            if direction1 == 'right':
                right1.append(pygame.Rect(pplayer1.right-(SIZE1/2), pplayer1.centery-(BULLETSIZE/2), BULLETSIZE, BULLETSIZE))
            if direction1 == 'up':
                up1.append(pygame.Rect(pplayer1.centerx-(BULLETSIZE/2), pplayer1.top-BULLETSIZE, BULLETSIZE, BULLETSIZE))
            if direction1 == 'down':
                down1.append(pygame.Rect(pplayer1.centerx-(BULLETSIZE/2), pplayer1.bottom, BULLETSIZE, BULLETSIZE))
            BULLETINTERVAL1 = 0
        if fireGun2 and BULLETINTERVAL2 == BULLETGAP:
            if direction2 == 'left':
                left2.append(pygame.Rect(pplayer2.left+(SIZE2/2)-BULLETSIZE, pplayer2.centery-(BULLETSIZE/2), BULLETSIZE, BULLETSIZE))
            if direction2 == 'right':
                right2.append(pygame.Rect(pplayer2.right-(SIZE2/2), pplayer2.centery-(BULLETSIZE/2), BULLETSIZE, BULLETSIZE))
            if direction2 == 'up':
                up2.append(pygame.Rect(pplayer2.centerx+(SIZE2/10)-(BULLETSIZE/2), pplayer2.top-BULLETSIZE, BULLETSIZE, BULLETSIZE))
            if direction2 == 'down':
                down2.append(pygame.Rect(pplayer2.centerx+(SIZE2/10)-(BULLETSIZE/2), pplayer2.bottom, BULLETSIZE, BULLETSIZE))
            BULLETINTERVAL2 = 0
        ...
        if ITEMCOUNTER >= ITEMTIME:
            ...
            if randem == 0:
                ITEM0.append(pygame.Rect(random.randint(0,WINDOWWIDTH), random.randint(0,WINDOWHEIGHT), ITEMSIZE, ITEMSIZE))
            if randem == 1:
                ITEM1.append(pygame.Rect(random.randint(0,WINDOWWIDTH), random.randint(0,WINDOWHEIGHT), ITEMSIZE, ITEMSIZE))
            if randem == 2:
                ITEM2.append(pygame.Rect(random.randint(0,WINDOWWIDTH), random.randint(0,WINDOWHEIGHT), ITEMSIZE, ITEMSIZE))
            if randem == 3:
                ITEM3.append(pygame.Rect(random.randint(0,WINDOWWIDTH), random.randint(0,WINDOWHEIGHT), ITEMSIZE, ITEMSIZE))
            if randem == 4:
                ITEM4.append(pygame.Rect(random.randint(0,WINDOWWIDTH), random.randint(0,WINDOWHEIGHT), ITEMSIZE, ITEMSIZE))
            if randem == 5:
                ITEM5.append(pygame.Rect(random.randint(0,WINDOWWIDTH), random.randint(0,WINDOWHEIGHT), ITEMSIZE, ITEMSIZE))
    ...
    pygame.display.update()
    mainClock.tick(SPEED)
# ITEM: 0:상대 커짐, 1:아이스 총알, 2:해골, 3:레이저, 4:포탑, 5:회복
